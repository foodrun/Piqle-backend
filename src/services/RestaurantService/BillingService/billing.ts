...
export class BillingService {
  ...
  calculateCategoryWiseBill(consumable_type: string): void {
    ...
    this._totalOrderBill = this._foodOrderBill + this._drinksOrderBill;
  }

  Getter(bill: string): number | ICategorwiseBill {
    if (bill === Bill.TOTAL) return this._totalOrderBill;
    if (bill === Bill.FOOD) return this._foodOrderBill;
    if (bill === Bill.DRINKS) return this._drinksOrderBill;
    if (bill === Bill.FOOD_CATEGORIES) return this._categoryWiseFoodBill;
    if (bill === Bill.DRINKS_CATEGORIES) return this._categoryWiseDrinksBill;
